    ...
    (setNumber: number) => {
      ...
      if (set.completed) {
        ...
        if (set.time <= 0) {
          Alert.alert(
            "Invalid Input",
            "Please enter valid duration before completing the set."
          );
          return;
        }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headers}>
        ...
        <CustomText style={styles.headerText}>Duration</CustomText>
...
const SetForm = ({ setData, onUpdate, onComplete }: SetFormProps) => {
  const { time, completed, setNumber, previous } = setData;
...
export default DistanceOnlyLoggingForm;
