...
const SetsLoggingForm = ({
  ...
  const handleSetUpdate = useCallback(
    (setNumber: number, updates: Partial<SetData>) => {
      ...
      // Update in store - for reps-only logging, we set weight to 0
      ...
      if (updatedSet) {
        upsertSetByScheduleAndExerciseId(scheduleId, exerciseId, {
          ...
          weight: 0, // No weight for reps-only exercises
  ...
  const handleSetComplete = useCallback(
    (setNumber: number) => {
      ...
      if (set.completed) {
        ...
        // Mark as complete - validate first (only check reps since no weight)
        if (set.reps <= 0) {
          Alert.alert(
            "Invalid Input",
            "Please enter valid reps before completing the set."
          );
          return;
        }
...
const SetForm = ({ setData, onUpdate, onComplete }: SetFormProps) => {
  const { reps, completed, setNumber, previous } = setData;
...
export default SetsLoggingForm;
